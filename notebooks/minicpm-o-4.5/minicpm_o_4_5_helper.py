...
def patch_stateful_apm(ov_model):
    ...
    not_kv_inputs = [inp for inp in ov_model.inputs 
                     if not any('past_key_values' in n for n in inp.get_names())]
...
def convert_tts_code_head(model, output_path: Path):
    ...
    class TTSCodeHead(nn.Module):
        ...
        def forward(self, hidden_states):
            ...
            logits = torch.empty(
                batch_size, seq_len, num_audio_tokens, self.num_vq,
...
def convert_token2wav(
    ...
    flow.load_state_dict(
        torch.load(f"{audio_tokenizer_path}/flow.pt", map_location="cpu", weights_only=True), 
        strict=True
    )
    flow.cpu().eval()
    ...
    hift_state_dict = {
        ...
        for k, v in torch.load(f"{audio_tokenizer_path}/hift.pt", map_location="cpu", weights_only=True).items()
    }
        ...
        if self.past_key_values is not None:
            ...
            for input_name in self.key_value_input_names:
                ...
                inputs[input_name] = ov.Tensor(
                    model_input.get_element_type(), shape.get_shape())
...
class OVResampler:
    ...
    def __init__(self, model_path, device, embed_dim=4096, max_size=(70, 70)):
        ...
        self.pos_embed_base = get_2d_sincos_pos_embed(
            embed_dim, self.max_size
        )
    ...
    def __call__(self, x, tgt_sizes):
        ...
        for i in range(bs):
            ...
            pos_embed_2d = self.pos_embed[:tgt_h.item(), :tgt_w.item(), :].reshape(
                num_patches, -1
            )
        ...
        if use_cache:
            ...
            for i in range(self.num_layers):
                key = torch.from_numpy(
                    self.request.get_tensor(f"present.{i}.key").data.copy()
                )
                value = torch.from_numpy(
                    self.request.get_tensor(f"present.{i}.value").data.copy()
                )
...
class OVLLMForCausalLM(GenerationMixin):
    ...
    def __init__(self, model_path, device, config):
        ...
        self._ov_embedding = OVLLMEmbedding(
            Path(model_path) / LLM_EMBEDDING_NAME, device
        )

        # Load language model (auto-detects stateful/stateless)
        self._ov_language = OVLLMLanguageModel(
            Path(model_path) / LLM_LANGUAGE_NAME, device
        )
...
class OVMiniCPMO:
    ...
    def init_vision_module(self):
        ...
        """
        vision = OVVisionModel(
            self.model_path / VISION_NAME, self._ov_device
        )
    ...
    def init_resampler(self, embed_dim, vision_dim):
        ...
        resampler = OVResampler(
            self.model_path / RESAMPLER_NAME,
            self._ov_device,
            embed_dim=embed_dim,
            max_size=(70, 70)
        )
    ...
    def init_audio_module(self):
        ...
        audio_encoder = OVAudioEncoder(
            self.model_path / AUDIO_ENCODER_NAME, self._ov_device
        )
        print("  ✅ Audio Encoder loaded")
        
        audio_projection = OVAudioProjection(
            self.model_path / AUDIO_PROJECTION_NAME, self._ov_device
        )
        print("  ✅ Audio Projection loaded")
        
        pool_step = getattr(self.config, 'audio_pool_step', 5)
        audio_avg_pooler = torch.nn.AvgPool1d(
            kernel_size=pool_step, stride=pool_step, ceil_mode=True
        )
    ...
    def init_tts_module(self):
        ...
        """
        tts_embedding = OVTTSEmbedding(
            self.model_path / TTS_EMBEDDING_NAME, self.tts_device
        )
        tts_llm = OVTTSLanguageModel(
            self.model_path / TTS_LANGUAGE_NAME, self.tts_device
        )
        tts_projector_spk = OVTTSProjectorSpk(
            self.model_path / TTS_PROJECTOR_SPK_NAME, self.tts_device
        )
        tts_projector_semantic = OVTTSProjectorSemantic(
            self.model_path / TTS_PROJECTOR_SEMANTIC_NAME, self.tts_device
        )
        tts_code_embedding = OVTTSCodeEmbedding(
            self.model_path / TTS_CODE_EMBEDDING_NAME, self.tts_device
        )
        tts_code_head = OVTTSCodeHead(
            self.model_path / TTS_CODE_HEAD_NAME, self.tts_device
        )
    ...
    def get_audio_embedding(self, data, chunk_length=-1, dummy=True):
        ...
        if len(wavforms) > 0:
            ...
            audio_states = self.apm(
                wavforms.to(self.dtype), audio_attention_mask, use_cache=False
            )
            ...
            input_lengths_after_pooling = (
                input_lengths_after_cnn - pool_step
            ) // pool_step + 1
        ...
        if len(wavforms) > 0:
            ...
            if apm_is_stateful:
                ...
                if cache_length + max_seq_len >= apm_max_len:
                    import logging
                    logging.warning(
                        f"audio cache length {cache_length + max_seq_len} exceed {apm_max_len}, reset."
                    )
                    self.apm.reset_state()
                    cache_length = 0
            else:
    ...
    def llm_forward(self, inputs_embeds, attention_mask, position_ids):
        ...
        """
        logits, hidden_states = self.llm._ov_language(
            inputs_embeds.to(self.dtype),
            attention_mask,
            position_ids
        )
...
class OVStreamDecoder:
    ...
    def __init__(
        ...
        custom_special_tokens = [
            "<unit>", "</unit>", "<image>", "</image>", "<slice>", "</slice>",
            "<|listen|>", "<|speak|>", "<|tts_bos|>", "<|tts_eos|>",
            "<|audio_start|>", "<|audio_end|>", "<|chunk_eos|>",
            "<|chunk_tts_eos|>", "<|turn_eos|>",
        ]
    ...
    def register_system_prompt_with_context(
        ...
        self._previous_marker_token_ids = (
            self.tokenizer.encode(context_previous_marker, add_special_tokens=False)
    ...
    def feed(self, embeds: torch.Tensor, return_logits: bool = False):
        ...
        position_ids = torch.arange(
            past_len + self._position_offset,
            past_len + self._position_offset + L,
            dtype=torch.long
        ).unsqueeze(0)
...
class OVMiniCPMODuplex:
    ...
    def from_existing_model(
        ...
        **kwargs,
        ...
        instance.decoder.set_window_config(DuplexWindowConfig(
        ...
        if instance.generate_audio:
            ...
            ) if callable(globals().get('gen_logits', None)) else None
    ...
    def _init_decoder(self):
        ...
        self.decoder.set_window_config(DuplexWindowConfig(
            ...
            if self.processor is not None and hasattr(self.processor, 'process_audio_streaming'):
                ...
                if audio_embeds_list and len(audio_embeds_list) > 0:
                    audio_embeds = torch.cat(
                        [t for g in audio_embeds_list for t in g], dim=0
                    ) if audio_embeds_list else None
            ...
            if last_id.item() in self.chunk_terminator_token_ids:
                ...
                if self.ls_mode == "explicit":
                    logits, _ = self.decoder.feed(
                        self.decoder.embed_token(last_id.item()), return_logits=True
                    )
                break
            else:
        ...
        if self.model is not None and self.model.token2wav is not None:
            if new_tokens is not None:
                audio_waveform = self._generate_waveform_from_tokens(
                    new_tokens, prompt_wav_path or self.prompt_wav_path, end_of_turn, force_flush=force_flush
                )
    ...
    def _convert_results_to_tts_input(self, results):
        ...
        if len(results) == 0:
            ...
            audio_bos = self.model.tts.embedding(
    ...
    def _tts_generate_chunk(
        ...
        new_tokens = torch.zeros(
            batch_size, max_new_token, num_vq, dtype=torch.long
        )
    ...
    def inference(self, speech_feat: torch.Tensor) -> torch.Tensor:
        ...
        if self.hift_input_len > 0:
            ...
            if original_len < target_len:
                ...
                mel_input = np.pad(mel_input, ((0, 0), (0, 0), (0, pad_len)),
