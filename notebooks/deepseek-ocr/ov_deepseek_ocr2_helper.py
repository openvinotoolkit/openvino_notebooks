...
from DeepSeek_OCR_2.conversation import get_conv_template
from abc import ABC
import math
...
SAM_MODEL_PATH = "openvino_sam_model.xml"
QWEN2_MODEL_PATH = "openvino_qwen2_model.xml"
...
def convert_deepseek_ocr2(model_id=model_id, model_path=None, quantization_config=None):
    ...
    if all((model_path / model_name).exists() for model_name in [QWEN2_MODEL_PATH, SAM_MODEL_PATH, TEXT_EMBEDDINGS_PATH, LANGUAGE_MODEL_PATH]):
    ...
    config.query_768 = pt_model.model.qwen2_model.query_768.weight.tolist()
    config.query_1024 = pt_model.model.qwen2_model.query_1024.weight.tolist()
    ...
    if not (model_path / QWEN2_MODEL_PATH).exists():
        print("⌛ Convert Qwen2 model")

        patch_cos_sin_cached_fp32(pt_model.model.qwen2_model.model.model)

        def vision_forward(self, x_combined, token_type_ids, n_query):
            y = self.qwen2_model.model(inputs_embeds=x_combined, token_type_ids=token_type_ids)[0]
            y = y[:, n_query:, :]  # causal flow query
            features = self.projector(y)
            return features

        pt_model.model._orig_forward = pt_model.model.forward
        pt_model.model.forward = types.MethodType(vision_forward, pt_model.model)
        moe_intermediate_size = config.language_config["moe_intermediate_size"]
        dummy_inputs = {
            "x_combined": torch.ones([1, 512, moe_intermediate_size]),
            "token_type_ids": torch.cat([torch.zeros(1, 256, dtype=torch.long), torch.ones(1, 256, dtype=torch.long)], dim=1),
            "n_query": torch.tensor(256, dtype=torch.long),
        }

        input_shapes = [
            ov.PartialShape([-1, -1, moe_intermediate_size]),  # x_combined
            ov.PartialShape([-1, -1]),  # token_type_ids
            ov.PartialShape([1]),  # n_query
        ]
        ov_model = ov.convert_model(pt_model.model, example_input=dummy_inputs, input=input_shapes)
        ...
        ov.save_model(ov_model, model_path / QWEN2_MODEL_PATH)
        del ov_model
        pt_model.model.forward = pt_model.model._orig_forward
        del pt_model.model._orig_forward
        cleanup_torchscript_cache()
        gc.collect()
        print("✅ Qwen2 model successfully converted")

    if not (model_path / SAM_MODEL_PATH).exists():
        print("⌛ Convert SAM model")
        ov_model = ov.convert_model(pt_model.model.sam_model, example_input=torch.ones([1, 3, 1024, 1024]))

        if quantization_config is not None and "vision" in quantization_config:
            nncf.compress_weights(ov_model, **quantization_config["vision"])
        ov.save_model(ov_model, model_path / SAM_MODEL_PATH)
        del ov_model
        cleanup_torchscript_cache()
        gc.collect()
        print("✅ SAM model successfully converted")
    ...
    if not (model_path / LANGUAGE_MODEL_PATH).exists():
        ...
        input_shapes += [ov.PartialShape([-1, -1, lm.config.hidden_size])]  # inputs_embeds
        ...
        ov_model = ov.convert_model(ts_decoder, example_input=dummy_inputs, input=input_shapes)
...
class OVDeepseekOCR2ForCausalLM(GenerationMixin):
    def __init__(self, model_dir, device="CPU", ov_config=None):
        from DeepSeek_OCR_2.modeling_deepseekocr2 import DeepseekOCR2Config

        self.config = DeepseekOCR2Config.from_pretrained(model_dir)
        self.generation_config = GenerationConfig.from_model_config(self.config)
        ...
        if model_dir is not Path:
            model_dir = Path(model_dir)
        self.qwen_2_model = core.compile_model(model_dir / QWEN2_MODEL_PATH, device)
        self.sam_model = core.compile_model(model_dir / SAM_MODEL_PATH, device)
        self.query_768 = torch.tensor(self.config.query_768)
        self.query_1024 = torch.tensor(self.config.query_1024)
        self.view_seperator = torch.tensor(self.config.view_seperator)

    def extract_image_features(self, patches):
        x = torch.from_numpy(self.sam_model(patches)[0])
        x = x.flatten(2).transpose(1, 2)  # (bs, spatial, channels)
        bs, n_query, _ = x.shape
        if n_query == 144:
            param_img = self.query_768
        elif n_query == 256:
            param_img = self.query_1024
        else:
            raise ValueError(f"Unsupported n_query={n_query}. Expected 144 or 256.")
        batch_query_imgs = param_img.unsqueeze(0).expand(bs, -1, -1)

        # Combine image and query embeddings
        x_combined = torch.cat([x, batch_query_imgs], dim=1)

        # Create token type IDs (0 for image, 1 for query)
        ...
        features = torch.from_numpy(self.qwen_2_model([x_combined, token_type_ids, n_query])[0])
        return features
        ...
        for image, crop_shape in zip(images, images_spatial_crop):
            ...
            with torch.no_grad():
                ...
                if torch.sum(patches).item() != 0:
                    ...
                    crop_flag = 1
                    local_features = self.extract_image_features(patches)
                    global_features = self.extract_image_features(image_ori)
