"""
Gradio helper for Qwen3-ASR with OpenVINO GenAI (ASRPipeline) and the optimum-intel forced aligner.
...
def make_demo(asr_pipe, aligner_model=None, aligner_processor=None):
    """
    Create a Gradio demo for Qwen3-ASR with OpenVINO GenAI.

    Args:
        asr_pipe: an ``openvino_genai.ASRPipeline`` created from a Qwen3-ASR OpenVINO IR.
        aligner_model: optional ``OVModelForQwen3ASRForcedAligner`` for word-level timestamps.
        aligner_processor: the ``AutoProcessor`` for the aligner (handles
            ``prepare_forced_aligner_inputs`` / ``decode_forced_alignment``). Required when
            ``aligner_model`` is provided.
    """
    ...
    def transcribe(audio_upload: Any, lang_disp: str, progress=gr.Progress(track_tqdm=True)):
        """Transcribe (and optionally align) the uploaded audio with OpenVINO GenAI."""
        ...
        # 1. Transcribe with the OpenVINO GenAI ASR pipeline.
        raw_speech = wav.astype(np.float32).tolist()
        try:
            result = asr_pipe.generate(raw_speech, language=language) if language else asr_pipe.generate(raw_speech)
        except Exception:
            # Fall back to automatic language detection if the language hint is not accepted.
            result = asr_pipe.generate(raw_speech)
        ...
        timestamps = None
        if aligner_model is not None and aligner_processor is not None and transcription.strip():
            try:
