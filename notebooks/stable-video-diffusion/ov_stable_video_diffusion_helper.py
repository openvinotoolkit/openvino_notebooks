...
def convert_stable_video_diffusion():
    ...
    if load_pt_pipeline:
        ...
        pipe = StableVideoDiffusionPipeline.from_pretrained(
            "stabilityai/stable-video-diffusion-img2vid-xt", variant="fp16", scheduler=noise_scheduler, torch_dtype="float16"
        )
        hf_hub_download(
    ...
    if not IMAGE_ENCODER_PATH.exists():
        with torch.no_grad():
            ts_decoder = TorchScriptPythonDecoder(image_encoder, example_input=torch.zeros((1, 3, 224, 224)), trace_kwargs={"check_trace": False})
    ...
    if not VAE_DECODER_PATH.exists():
        ...
        with torch.no_grad():
            ts_decoder = TorchScriptPythonDecoder(
                vae_decoder, example_input=(torch.zeros((8, 4, 36, 64)), torch.tensor(8)), trace_kwargs={"check_trace": False}
            )
    ...
    if not VAE_ENCODER_PATH.exists():
        vae.forward = lambda sample: {"latent_parameters": vae.encode(x=sample)["latent_dist"].parameters}
        vae.eval()
        __make_16bit_traceable(vae)
        with torch.no_grad():
            ts_decoder = TorchScriptPythonDecoder(vae, example_input=torch.zeros((1, 3, 256, 256)), trace_kwargs={"check_trace": False})
            ov_model = ov.convert_model(ts_decoder)
    ...
    if not UNET_PATH.exists():
        ...
        with torch.no_grad():
            ts_decoder = TorchScriptPythonDecoder(unet, example_input=unet_inputs, trace_kwargs={"check_trace": False})
            ov_model = ov.convert_model(ts_decoder)
...
class OVStableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_vae_image(
        ...
        image_latents = DiagonalGaussianDistribution(torch.from_numpy(self.vae_encoder(image)[0])).sample(generator=generator)
    ...
    def __call__(
        ...
        image_latents = self._encode_vae_image(image, device, num_videos_per_prompt, do_classifier_free_guidance, generator=generator)
