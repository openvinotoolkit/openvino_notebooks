...
> **Device placement defaults**: the transformer (run several times per step) defaults to `AUTO` so it can use the GPU; the **VAE** defaults to `AUTO` too. The **text encoder defaults to CPU** because the UMT5 encoder must run in fp32 — it overflows in fp16, producing a black image — and compiling the ~5 GB fp32 model on GPU takes ~80 s, whereas on CPU it compiles in ~2 s and only runs twice per generation. When the text encoder or VAE run on GPU/NPU, `load_ov_pipeline` automatically applies an fp32 precision hint so the output matches CPU; pass `ov_config` to override.
