...
> **Device placement defaults**: the transformer and the **VAE** default to `AUTO` and run in **fp16** on GPU — verified to match the CPU output (no black image). The VAE's multi-frame **video** decode on GPU needs the OpenVINO `>= 2026.3` nightly (its GPU plugin spills the large activation buffer to host); image decode works on the stable release too. The **text encoder defaults to CPU** because the UMT5 encoder must run in fp32 — it overflows in fp16, producing a black image — and compiling the ~5 GB fp32 model on GPU takes ~80 s, whereas on CPU it compiles in ~2 s and only runs twice per generation. `load_ov_pipeline` applies the right per-component precision hint automatically on GPU/NPU; pass `ov_config` to override.
