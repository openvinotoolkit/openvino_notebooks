...
def build_state_initializer(ov_model: ov.Model, batch_dim: int):
    ...
    # Map variable_id -> partial shape of Assign source. Assign is fed by the
    # computational graph (Concat/SDPA outputs), where static dims like
    # num_heads / head_dim are preserved even when the corresponding ReadValue
    # output shape lost staticity after MakeStateful.
    var_id_to_shape = {}
        ...
        if op.get_type_name() == "ReadValue":
            ...
            for i, dim in enumerate(ref_shape):
                if i == batch_dim:
                    ...
                    # truly dynamic dim (e.g. seq_len) -> initial cache is empty
