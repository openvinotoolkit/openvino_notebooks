 ...
 * Original model: [Qwen3.6-35B-A3B](https://huggingface.co/Qwen/Qwen3.6-35B-A3B)

## Description
This is [Qwen3.6-35B-A3B](https://huggingface.co/Qwen/Qwen3.6-35B-A3B) model converted to the [OpenVINO™ IR](https://docs.openvino.ai/2025/documentation/openvino-ir-format.html) (Intermediate Representation) format with weights compressed to INT4 by [NNCF](https://github.com/openvinotoolkit/nncf).
...
* OpenVINO version 2026.2.0 and higher (currently available only as a [nightly build](https://storage.openvinotoolkit.org/simple/wheels/nightly))
* Optimum Intel 1.27.0 and higher (currently available only from the [main branch on GitHub](https://github.com/huggingface/optimum-intel))
...
> **Note.** The released `optimum-intel` and `openvino` packages do not yet support the `qwen3_5_moe` model type and the released `optimum-intel` depends on `transformers.onnx` (removed in `transformers>=5.0`), so installing `optimum[openvino]` will fail with `ModuleNotFoundError: No module named 'transformers.onnx'` and/or `ValueError: ... model type 'qwen3_5_moe' ...`. Install `optimum-intel` from the `main` branch, the nightly build of `openvino`, and `transformers>=5.5.0` as shown above.
...
model_id = "OpenVINO/Qwen3.6-35B-A3B-int4-ov"
...
model_id = "OpenVINO/Qwen3.6-35B-A3B-int4-ov"
model_path = "Qwen3.6-35B-A3B-int4-ov"
...
Check the original [model card](https://huggingface.co/Qwen/Qwen3.6-35B-A3B) for limitations.

## Legal information

The original model is distributed under [Apache License Version 2.0](https://huggingface.co/Qwen/Qwen3.6-35B-A3B/blob/main/LICENSE) license. More details can be found in [Qwen3.6-35B-A3B](https://huggingface.co/Qwen/Qwen3.6-35B-A3B).
